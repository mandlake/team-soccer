...
FROM player
         JOIN team ON player.team_id = team.team_id
WHERE (team_name = '드래곤즈' OR team_name = '삼성블루윙즈')
  AND POSITION = 'MF'
...
SELECT player_name, IFNULL(NULLIF(height, ''), 0), IFNULL(NULLIF(weight, ''), 0)
FROM player
         JOIN team ON player.team_id = team.team_id
...
SELECT player_name,
       POSITION,
       CONCAT(IFNULL(NULLIF(height, ''), 0), 'CM'),
       CONCAT(IFNULL(NULLIF(weight, ''), 0), 'KG'),
